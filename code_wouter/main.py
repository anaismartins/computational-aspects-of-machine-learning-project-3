...
data = np.load("dataset_blip_injection_koyfish_weighted_average_h1.npy")
X = data[:,:-1]
y = data[:,-1]
...
batch_size = round(len(train_data.tensors[0])/100)
...
model = VariableNet(n_units, n_layers, n_classes)
m = "VariableNet"
print(model)
